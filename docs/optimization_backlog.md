...
- **动机**：同一场比赛的多个 Polymarket 市场（赛果 vs 首盘）应概率一致——不一致 = 套利/信号；
  相关市场的定价也可互参。
- **延伸（2026-06-14 流动性 census 修正——旧假设被推翻）**：曾设想"赛果市场流动性厚得多→迁过去"。**实测同场配对（151 个当前赛事）证伪：同一场比赛首盘盘流动性厚 140/151=93%（首盘中位 $215 vs 赛果 $0）；当前 Polymarket 网球的活流动性压倒性在首盘。** 之前"赛果 $43k"是 2024 大满贯已结算残值的混杂。**结论：留首盘（A17 也无 edge 理由迁赛果）。** A16 的真正可做形态 = **同场「首盘+赛果」bet-both / 跨市场互参**（82% 当前赛事两类盘都挂），但赛果当前中位 $0 基本空盘 → **只有大满贯级赛果盘才深（$41k），bet-both 是大赛 niche + mock 相向前采集**（同场两腿赛前价数学上绑定，不一致=信号；同场下两腿亦可分散方差，后者属 A14 风控、可在 backtest 相用）。
...
- **流动性补 census（2026-06-14，实测定论）**：同场配对实测 → **首盘比赛果厚 93%（同场 140/151；首盘中位 $215 vs 赛果 $0）**，当前活流动性在首盘。**∴ 首盘 vs 赛果 = 留首盘**（无 edge 也无流动性理由迁）；bet-both 是大赛 niche（详见 A16 延伸）。纠正了"赛果更厚"的旧假设（那是 2024 大满贯已结算残值混杂）。

### A18 · 赛果共识反推首盘信号（用户洞察：留首盘 + 用赛果当外部信号）— `偏正未坐实`（2026-06-14；下一闸=成本后 ROI）
- **想法（用户 2026-06-14）**：Polymarket 可成交流动性在首盘，但数据/效率在赛果世界（Pinnacle/大盘共识）。`P(赢单盘)` 由 `P(赢整场)` 数学反推 → **「赛果共识反推的首盘概率」当首盘市场的外部参考信号**。不迁市场、不 bet both，而是把赛果共识喂回 agent 的首盘 bet。若比 Polymarket 薄首盘价更准 = **公开信息、可回测的 edge，在我们本来就交易的市场上 → 回测就能"赚而非仅活"**（修正"backtest edge≈0"框架）。与 A13 不同：A13 用的是我方弱 Sackmann 信号，这条用的是**全市场赛果共识**（强得多）。
- **落地**（commit d826ede）：`agent/backtest/sharp_line.py:match_to_set_prob`（反解 bo3/bo5，iid sets，7 测试）+ `scripts/probe_setprob_signal.py`（de-vig AvgW/AvgL 共识→反推首盘概率，cluster bootstrap+SESOI，复用 A17 orientation/cassette-ledger，`--progress-file`）。
- **结果（真实数据 2024-26，n=1704/61簇，仅丢 20 void，功效十足）**：点估 **+0.00353 Brier**（赛果共识反推 比 Polymarket 首盘价更准，且 > SESOI 0.002）；**cluster CI [-0.0011, +0.0090] 含 0 → INCONCLUSIVE**（偏正、刚好不显著）；iid CI [-0.0003,+0.0076] 几乎全正；**两市场分歧大：平均 4.9%，389/1704=23% 分歧≥5%**。
- **判断**：整弧（A13/A17 均否）**最接近真 edge 的结果，但"像有、没坐实"**，且 +0.0035 太小、很可能扛不住 fee+spread。**有苗头非绿灯。** 数据坑：2026 Pinnacle 仅 5-8% 填充→改用 Avg 共识（100%）；小样本(PS,n=110)点估 +0.0166 被夸大 5x。
- **下一道便宜闸（去向）**：**成本后 ROI**——照「分歧时押向共识」模拟下注、扣 fee+spread 后是否净正（小 Brier 优势能否变成钱）。需把 betting sim 加进探针或缓存每场数据离线算。⚠️ 探针密集 HTTP IO 会拖垮整机工具路径，**别由 Claude 会话后台跑，交终端/挑空闲时**。
