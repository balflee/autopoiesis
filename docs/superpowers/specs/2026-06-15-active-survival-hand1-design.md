...
4. **回归 + 地板诚实**：g1（真实）仍吃下其（过拟合）edge；g2（打乱）仍**正确最小化损失**。探索地板的**价值**在**隐藏-edge 世界**（信号低于门但真有 edge）由 `mean_final_breath` 提升坐实（此 runtime 纯弃权者不死，故用呼吸而非死亡率）；**纯噪声世界的探索成本为正**（loss×m 物理使然），据实**记录、不假装为零**。
