...
- [ ] **Step 2: Failing tests** — EV within Monte-Carlo tolerance `abs(agent_ev−edge) < 3·0.5/√n` for edge 0.0 and 0.10 (`winning_price==1.0` is the only exact-equality check); subgate rows abstain (assert a v3-seeded `DecisionEngine.decide()` returns `NO_BET_NO_EDGE` on a subgate row); `quick_numerical_deaths(build_synthetic_world(400,0.0,1)..., loss_multiplier=3.0, initial_breath=20.0, max_lives=6, fragile_max_breath_risk_pct=0.95) > 1` (a death-inducing regime; this test only proves mid-schedule settlement, independent of the calibration window); deterministic.
...
- [ ] **Step 4: Implement.**
  ...
  - `quick_numerical_deaths(rows, snaps, *, loss_multiplier, initial_breath, max_lives, fragile_max_breath_risk_pct=0.95)`: → `run_survival_over_rows(..., fragile_max_breath_risk_pct=fragile_max_breath_risk_pct)["summary"]["deaths"]` (forward the same risk regime Task 3/deployment use).
...
- [ ] **Step 1:** Create `survival_metrics.py` (`journey_metric` + `groundhog_metric`, each with a shape assertion). Write the failing test. **First EMPIRICALLY pin a feasible point** via `quick_numerical_deaths`: find a concrete `(m, initial_breath, n_rows)` where the 0-edge above-gate world has `death_rate>0.5` AND the +edge world has `death_rate<0.5` (because wins replenish breath, the dual window is NOT guaranteed at any `m<5` — if the default `initial_breath=35` doesn't separate, **tune `initial_breath` DOWN** (e.g. 20), and record the chosen point). **JOINT CONSTRAINT (load-bearing): cap the `loss_multiplier` grid below the floor-value breakeven — feasible window `m ∈ (1, 1.5)`** (`m>1` kills the 0-edge world; `m<1.5` keeps Task 6(b) floor-value feasible), so the SAME recommended `m` serves calibration AND validation. The test asserts the pinned point separates AND `calibrate(...)` recommends an `m` from the grid; "no m in (1,1.5) separates" is a surfaced calibration FAILURE (co-tune `initial_breath`/`n_rows` down), not a crash.
...
(groundhog ONLY.) `run_groundhog_export(..., exploration_epsilon=0.0)` keyword-only; `base_seed=dataclasses.replace(base_seed, exploration_epsilon=...)`. **Two explicit edits:** (i) thread `exploration_rng=random.Random(incarnation_k) if exploration_epsilon>0 else None` into the in-sample **`run_survival_season(...)` call at `reincarnation.py:1479`** (groundhog drives lives via `run_survival_season`, which forwards to its `:1739` learner — NOT `_build_life_loop` directly); the holdout (`_run_frozen_holdout`, `:871`) stays `None`. (ii) append `"exploration_epsilon": exploration_epsilon` to the `knobs` dict at `:1828-1835` (knobs is built from params, not the seed — the test reads `knobs["exploration_epsilon"]`). CLI flags `--loss-multiplier/--initial-breath/--exploration-epsilon/--synthetic-edge`; `--synthetic-edge E` SKIPS the `:118-124` cache load; replace `:156-157` literals; defaults from calibration JSON, fail-closed if requested+absent; `--design passes`+active-flag ⇒ error. Test (keyword-only, SUBGATE world): `artifact["knobs"]["loss_multiplier"]==1.2`, `["exploration_epsilon"]==0.1`, `groundhog_metric(artifact).total_bets>0`. **Commit** `feat(reincarnation): groundhog calibrated economy + per-incarnation exploration + synthetic-edge`.
...
- **R7 (HIGH=2→1 distinct):** (1) **Floor-VALUE m-coupling made explicit (the unifying point)** — the probe is net-positive only when `m < true_prob/(1−true_prob)` (≈1.5 at edge=0.10); the recalibration target for honest ~55% survival is `m≈1.2` (break-even `m/(1+m)≈0.55`), which is `<1.5`, so the SAME low-m enables BOTH honest survival AND exploration value; legacy m=5 breaks both. **Calibration grid now capped to `m∈(1,1.5)`** so the recommended m serves calibration + validation jointly. (2) Breath is clamped `max(0,·)`+respawn-reset ⇒ `mean_final_breath` read **with `total_bets`+`death_rate`**, never alone. (3) `_value_side` returns `None` on neutral (`p_model==price`) too. (4) Task 5: thread rng into the in-sample `run_survival_season` at `reincarnation.py:1479` (holdout `:871` None) + append `exploration_epsilon` to the `knobs` dict at `:1828-1835`. (5) Task 1 EV uses a Monte-Carlo tolerance; `quick_numerical_deaths` forwards `fragile_max_breath_risk_pct=0.95` (regime-consistent with Task 3/deployment). (6) `_decision_engine_from_seed` header marks `exploration_rng` as Task-4-added.
