...
4. **A14 生效**：`desperate` 在回测被正确计算并传入；濒死时仓位**缩小**（不是放大）。
5. **A2 生效**：死亡产生一个**非零**学习梯度，导致下一世行为可度量地改变（朝更能活的方向）。
6. **回归**：g1（真实）仍能吃下其（过拟合）edge；g2（打乱）agent 仍**正确地最小化损失**（不被探索地板逼成更快地乱赌而死）。
...
**数学**：把单世 breath 动态建成带吸收壁的随机游走——给定胜率 `p`、每注 breath 增量（赢 `+w`、输 `−5w`，含 `loss_multiplier`）、租漏率（`every`/`cost`）、`initial_breath`、sizing，求**生存概率 / 期望寿命 = f(p, m, tithe, breath0, sizing)**。产出：**「探索成本 vs 探测力」「胜率 vs 期望寿命」权衡曲线 + 推荐参数集**。
