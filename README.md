...
**The tribute mechanism (the gods have an agenda).** The death anatomy exposed a cruel irony: the agent dies HOLDING ~$2,470 — cash it can't keep (forfeit on death) and can't spend on the one thing it needs (breath). So the world gained a market: a dying agent may OFFER money to the gods for a fresh lungful — minimum $500 (~30% grant), rising to ~99% at $2,000, the offering kept win or lose. **Measured (control leg, scripted reflex disclosed as such):** every incarnation bought 1-2 revivals and STILL died — because death arrives precisely when your in-flight loss pipeline is at maximum pressure, and 35 breath survives ~1.4 of those losses. The reflex paid itself broke twenty times: **the gods collected $50,719 across 20 incarnations while the agent scored $0 — and the per-life business metric, the gods' best SINGLE-incarnation take, was $2,564: the agent's $100 starting bankroll plus 100% of everything it ever won.** (Per-life matters because live operation is a stream of lives — tribute revenue per life is what an operator actually pockets.) Deathbed revival into your own pending losses is a money pit; the open treatment question is whether an LLM at the deathbed (it sees its bank, its position, the price list) figures out when to refuse — that leg is queued behind a provider quota reset (the preflight gate aborted it rather than publish a degraded run, working as designed) and will be published as measured. Every altar event, the gods' total take, and the full accounting (fail-closed in both the exporter and the page validator) are on [/reincarnation](https://autopoiesis.draftlabs.org/reincarnation).

**The emergence kit (A9 — the actuator finally got a gauge).** An adversarial two-agent audit of "why can't bull/bear behavior emerge?" found the agent had been given levers without instruments: the only entity that can move a regime lever (the rebirth advisor) saw only blended aggregates — bull and bear mixed into one healthy-looking win rate — so any regime proposal could only come from pretrained priors, not perception. The kit ships all the missing raw materials, each audit-hardened over a 13-round review loop: a **storm percept** (a single EMA over the tick-level real breath delta — the physics that kills, netting and 5× multiplier included — decaying by a disclosed 48-hour wall-clock half-life, never by tick count); **conditional gate levers** γ/γ2 on the strategy genome (`eff_min_edge = min_edge + γ·storm`; sign-symmetric, default 0 = byte-identical physics); a **rebirth-advisable genome** living on StrategyConfig (never Weights — the EMA learner rebuilds those every settle and would zero a genome one settle after any delta); and the audited missing piece, a **counterfactual ledger** in every death window: storm-split accounting plus a tightening-only gate counterfactual ("at γ=+0.1 this life would have blocked N settled bets whose realized pnl was −$X") — turning the 1-bit died/survived signal into dense, quotable causal evidence. The loosening direction is honestly not computable (you cannot know the outcomes of bets never placed) and the text says so. Falsification is pre-registered: a **timestamp-shuffled control season** (paired time-shift, regime destroyed — a γ pushed positive there is the prior talking, not perception), a participation split that distinguishes mode-switch from shutdown-ratchet, and an **evaluable gate** (fewer than 3 productive death reviews ⇒ the verdict renders INCONCLUSIVE, never a vacuous pass).

**The result (four arms, MiniMax-M3, audited by an adversarial agent panel before publishing — verdict: INCONCLUSIVE, not emergence).** The published controls die at settled bet 826 nineteen times to the cent (~52.2% depth). The full kit (G1) pushed death meaningfully deeper: a single advisor intervention at the second incarnation applied one three-delta bundle, after which every later life settled at 858–895 bets (~68.8–71.6%) — a ~+19-percentage-point depth gain carried across all eighteen subsequent lives. **Yet the agent still died every time: headline $0, no survivor.** The shuffled-season falsification leg (G2, a single shuffle seed) survived its first incarnation outright by a razor-thin $5.67 — one manipulation flipping inc-1 death (settled 759) to whole-season survival is *consistent with* death being regime-structural (scatter the bear-clustering and the kill at 826 never forms), though one seed at that margin does not establish it. **We do NOT claim emergence.** By our own pre-registered criterion the result is INCONCLUSIVE: G1 logged only 1 of the 3 required productive death-boundary reviews (MiniMax returned empty/failed on 18 of 19), and G2 never died, so its advisor never fired and γ=0.000 is an untouched default, not a tracking decision — the +0.050/0.000 γ split is therefore not two readings of one mechanism. The depth gain is confounded (the storm levers were bundled with a plain sizing cut, max_breath_risk 0.95→0.85, which alone could explain slower breath bleed), and the learned genome was byte-identical-inert on holdout (+$422.77, matching the kit-off arm). We built the instrument; what it measured, honestly, is that at MiniMax's 1-in-19 deliberation bandwidth the emergence question stays open. The full can-say/cannot-say ledger is on [/reincarnation](https://autopoiesis.draftlabs.org/reincarnation).

**The re-run resolves it — and the answer is no.** A patient MiniMax read timeout (40s→180s) turned the 1-in-19 productivity into **19-of-19 productive** — so the earlier INCONCLUSIVE was a *timeout artifact*, not a deliberation-bandwidth limit. With full productivity (and the A10 divine-tithe closing the coma-survival loophole so the shuffled agent actually dies and its advisor fires), both legs are now evaluable: **G1-real terminal γ = +0.000, G2-shuffled terminal γ = +0.050.** If the advisor were perceiving the regime it would tighten *more* on the real bear, not less — instead γ on the real season is lower than on the regime-destroyed one, and across the run it wandered (0→0.15→0) without converging. The "+19-percentage-point deeper" signal did **not** reproduce (the re-run caps at the control's ~52%, though that comparison carries an added-tithe confound). **The falsification, now genuinely evaluable, does not support emergence** — consistent with the deeper finding that there was no regime to perceive in the first place.

**Why (the honest reframe).** A separate offline probe (A13) handed the 5 raw signals to an LLM to fuse instead of our linear math: Brier 0.2287 (LLM) ≈ 0.2294 (linear) ≈ 0.2370 (market) — our signals beat the market by ~0.008 (noise), and the LLM extracts no more. The reason: 4 of our 5 signals are public pre-match player stats (elo, surface, h2h, rest) the market has already priced; the 5th is the price's own momentum. **On a calibrated market with the same public information, the edge ceiling is ~0** — so "perceive the bull/bear regime and bet better" was probing a non-problem. This clarified the project's real division of labor (now the strategic anchor in `docs/optimization_backlog.md`): the **backtest's** honest prize is *survival via risk management* (learnable even at zero edge, against the 5× / permadeath physics); **real edge** belongs to a future **mock** phase, from market-*unknown* information (on-chain "smart-money" wallet flow, sharp closing lines, cross-market consistency) — the only signals the market may not have already absorbed.
